...
Ray::Ray(Camera camera, glm::vec2 hitPos, glm::vec2 screenSize)
{
	...
	glm::mediump_f64mat4 projection = glm::perspective(camera.Zoom, (GLfloat)screenSize.x / (GLfloat)screenSize.y, 0.1f, 100.0f);
	//GLdouble modelview[16];
	//GLdouble projection[16];
	//glGetDoublev(GL_MODELVIEW_MATRIX, modelview);
	//glGetDoublev(GL_PROJECTION_MATRIX, projection);
	GLint viewport[4] = { 0, 0, screenSize.x, screenSize.y };
	gluUnProject(hitPos.x, screenSize.y - hitPos.y, 0, glm::value_ptr(view), glm::value_ptr(projection), viewport, &x, &y, &z);
	origin.x = x;
	origin.y = y;
	origin.z = z;
	gluUnProject(hitPos.x, screenSize.y - hitPos.y, 1, glm::value_ptr(view), glm::value_ptr(projection), viewport, &x, &y, &z);
	destination.x = x;
	destination.y = y;
	destination.z = z;
