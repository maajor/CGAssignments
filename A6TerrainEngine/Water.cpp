...
void Water::loadTexture(const GLchar* diffuse, const GLchar* normal, const GLchar* normal2){
	if (diffuse != NULL){
		int wid, hei;
	...
	if (normal != NULL){
		...
		glBindTexture(GL_TEXTURE_2D, texNormal);
		glUniform1i(glGetUniformLocation(this->waterShader.Program, "texture_normal"), 0);
	}
	if (normal2 != NULL){
		int wid, hei;
		...
		glBindTexture(GL_TEXTURE_2D, texNormal2);
		glUniform1i(glGetUniformLocation(this->waterShader.Program, "texture_normal2"), 0);
	}
}
...
void Water::render(GLuint skyTex){
	...
	glUniform1f(glGetUniformLocation(this->waterShader.Program, "_time"), (float) timer / 10000000000);
